...
test("risk envelope rejects excessive expected slippage",()=>{const v=validateIntent({symbol:"BTCUSDT",side:"Buy",qty:.01,referencePrice:100,leverage:1,marketAgeMs:1,spreadBps:1,slippageBps:100});assert.equal(v.ok,false);assert.ok(v.errors.includes("slippage"))});
test("risk envelope rejects inadequate liquidation buffer",()=>{const v=validateIntent({symbol:"BTCUSDT",side:"Buy",qty:.01,referencePrice:100,leverage:1,marketAgeMs:1,spreadBps:1,slippageBps:1,liquidationDistancePct:2});assert.equal(v.ok,false);assert.ok(v.errors.includes("liquidation_buffer"))});
